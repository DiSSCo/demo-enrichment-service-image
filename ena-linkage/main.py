...
def map_result_to_annotation(specimen_data: Dict, result: Dict[str, str],
        timestamp: str) -> Dict:
    """
    Map the result of the API call to an annotation
...
def run_api_call(specimen_data: Dict) -> List[Dict[str, str]]:
    ...
    if len(response_json) > 0:
        ...
        response_json = json.loads(response.content)
        if len(response_json) > 0:
            check_result(response_json, result_list, sequence_query,
                         specimen_data)
            return result_list
        else:
            logging.info(
...
def check_result(response_json: Dict, result_list: List[Dict[str, str]],
    ...
    for result in response_json:
        if run_additional_checks(result, specimen_data):
            result_list.append({'queryString': sequence_query,
                                'enaAccessionId': result['accession']})
